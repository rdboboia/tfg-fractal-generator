...
public class MandelbrotDynamicImageV5 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MandelbrotDynamicImageV5 frame = new MandelbrotDynamicImageV5();
	 ...
	 * Create the frame.
	 */
	public MandelbrotDynamicImageV5() {
		setTitle("V5");
...
//				int[] rgbPixelData = ((DataBufferInt) img.getRaster().getDataBuffer()).getData();
				...
				for (int i = 0; i < height; i++) {
					...
					for (int j = 0; j < width; j++) {
						...
						if (iteration < max) {
//							rgbPixelData[width * i + j] = Color.HSBtoRGB(iteration/256f, 1, iteration/(iteration+8f));
//							img.setRGB(j, i, Color.HSBtoRGB(iteration/256f, 1, iteration/(iteration+12f)));
...
//							System.out.println(brigthness);
							
							
							img.setRGB(j, i, Color.HSBtoRGB(iteration/(float)max, 1, brigthness/(float)maxBrightness));
